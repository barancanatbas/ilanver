...
func (a AddressRepo) Save(address *model.Adress) error {
	err := a.tx.Debug().Create(address).Error
...
func (a AddressRepo) GetAll() ([]model.Adress, error) {
	var address []model.Adress

	err := a.tx.Model(&model.Adress{}).Find(&address).Error

	return address, err
}

func (a AddressRepo) Update(address *model.Adress) error {
	err := a.tx.Model(address).Updates(map[string]interface{}{"detail": address.Detail, "districtfk": address.Districtfk}).Error
